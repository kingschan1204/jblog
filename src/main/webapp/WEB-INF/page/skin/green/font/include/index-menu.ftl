          ...
          <ul class="nav navbar-nav">
            ...
            <#if BLOG_CURRENT_USER??>
            	<li class="dropdown" id="menu_user_info">
	              ...
	              <#else>
	              	<li><a href="javascript:window.location='https://${host}/pub/login.do?url='+ window.location"><span class="glyphicon glyphicon-user"></span>&nbsp;<b>登录</b></a></li>
          </#if>
          </ul>
        </div><!--/.nav-collapse -->
      </div>
      			...
      			<div class="col-md-12">
		      		...
		      		<@CountCategory website_id="${website.id}">
				    	  <#list category_lis as category>
			    			<a href="${blogprefix}/category/${category.categoryName}" class="category_list">${category.categoryName}<span>(${category.total})</span></a>
				    	</#list>
				    </@CountCategory> 
		      		</div>
      				<div class="row">
